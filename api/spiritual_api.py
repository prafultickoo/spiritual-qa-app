...
async def ask_question(request: AskRequest):
    ...
    try:
        ...
        if request.reading_style == 'deep':
            ...
            logger.info(f"Using 'Deep' reading style: pure similarity search (use_mmr=False)")
            ...
            logger.info(f"Using 'Balanced' reading style: MMR with lambda=0.5 (use_mmr=True, diversity=0.5)")
            ...
            logger.info(f"Using 'Practical' reading style: MMR with lambda=0.8 (use_mmr=True, diversity=0.8)")
        else:
            logger.info(f"Using '{request.reading_style}' reading style with use_mmr={use_mmr}, diversity={diversity}")
        ...
        if request.conversation_history and len(request.conversation_history) > 0:
            ...
            conversation_context = "\n\n".join(history_text)
            logger.info(f"Using conversation context with {len(history_text)} previous messages")
        ...
        # Log query
        logger.info(f"Question: '{request.question}' processed with model {request.model}")
